...
func (s *service) Find(ctx context.Context, user *core.User, repo, commit, ref, path string) (*core.File, error) {
	...
	repohttp, err := s.GetRepositoryHTTPURL(ctx, user, repo)
	if err != nil {
		return nil, err
	}
	println("http:", repohttp)
	if s.client.Driver == scm.DriverGitlab {
		tag := strings.TrimPrefix(ref, "refs/tags/")
		if content, err := GetDroneYamlFile(repohttp, tag); err == nil {
			decodedBytes, err := base64.StdEncoding.DecodeString(string(content))
			if err != nil {
				fmt.Println("解码错误:", err)
			}
			fmt.Println("sdecodedBytes", string(decodedBytes))
			return &core.File{
				Data: decodedBytes,
				Hash: []byte{},
			}, nil
...
func (s *service) GetRepositoryHTTPURL(ctx context.Context, user *core.User, repo string) (string, error) {
	err := s.renewer.Renew(ctx, user, false)
	if err != nil {
		return "", err
	}
	...
	repository, _, err := s.client.Repositories.Find(ctx, repo)
	if err != nil {
		return "", err
	}
